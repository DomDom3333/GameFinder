...
{
    private static readonly GameFilterService GameFilterService = new();
    ...
    public async Task StartSession(string sessionCode, bool includeWishlist, int minOwners, int minWishlisted)
    {
        ...
        // Filter and randomize games using the service
        var filteredGames = GameFilterService.FilterAndRandomizeGames(
            session.Users,
            userId => UserGames.GetValueOrDefault(userId),
            userId => UserWishlists.GetValueOrDefault(userId),
            includeWishlist,
            minOwners,
            minWishlisted);

        session.CommonGames = filteredGames;
        await Clients.Group(sessionCode).SendAsync("SessionStarted", filteredGames);
    }
